...
// Valid VP8 frames for keepalive.
// Generated by ffmpeg: ffmpeg -f lavfi -i 'color=c=black:s=16x16' -c:v libvpx -f ivf
// These are real, decodable VP8 frames that any SFU will accept and forward.
var (
	// vp8Keyframe is a real VP8 keyframe (I-frame) for a 16x16 black image.
	// Generated by libvpx via ffmpeg. 31 bytes.
	vp8Keyframe = []byte{
	...
	// vp8Interframe is a real VP8 interframe (P-frame) for a 16x16 black image.
	// Generated by libvpx via ffmpeg. 21 bytes.
	vp8Interframe = []byte{
		0xb1, 0x01, 0x00, 0x01, 0x10, 0x10, 0x00, 0x18,
		0x00, 0x30, 0x3f, 0xf4, 0x0c, 0x00, 0x00, 0x00,
		0xfe, 0xff, 0xab, 0x50, 0x80,
	}
)

// MinSendInterval is the minimum time between consecutive WriteSample calls
// to avoid overloading the WebRTC stack (matches upstream behavior).
const MinSendInterval = 5 * time.Millisecond
...
func (t *Tunnel) Start(fps int) {
	...
	go func() {
		...
		// Always start with a keyframe — SFU won't forward video until it sees one.
		// Real VP8 encoders always emit a keyframe first.
		if t.SendFrame != nil {
		...
		for {
			...
			case data := <-t.sendQueue:
				// Enforce minimum interval between sends (matches upstream)
				if elapsed := time.Since(lastSend); elapsed < MinSendInterval {
				...
				// Reset keepalive timer after data send
			...
			case <-ticker.C:
				...
				// Determine keyframe interval: frequent keyframes during startup
				// so SFU recognizes the video track quickly, then normal interval.
				kfInterval := uint64(KeyframeInterval) // 60 normally
				if t.frameCount < 125 {                // first ~5s at 25fps
					kfInterval = 10 // keyframe every 10 frames (~0.4s)
				}

				var frame []byte
				if t.frameCount%kfInterval == 0 {
					frame = vp8Keyframe
				} else {
					frame = vp8Interframe
				}
