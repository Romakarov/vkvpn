...
func TestAuthCookie(t *testing.T) {
	...
	// Step 1: Login via query param to get session cookie
	loginReq := httptest.NewRequest("GET", "/api/status?token=testpass123", nil)
	loginW := httptest.NewRecorder()
	handler(loginW, loginReq)
	...
	// Step 2: Use session cookie for subsequent request
	req := httptest.NewRequest("GET", "/api/status", nil)
...
func TestApiHealth(t *testing.T) {
	...
	// Health endpoint should NOT expose internal info (version, uptime, etc.)
	if resp["version"] != nil {
		t.Fatal("health should not expose version")
	}
}
